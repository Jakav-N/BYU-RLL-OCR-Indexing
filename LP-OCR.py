...
# paths to pretrained models (also make sure to change these before returning code)
#PRIMA_CONFIG = r'V:\FHSS-JoePriceResearch\RA_work_folders\Ethan_Simmons\layoutParser\primaLayout\config.yaml'
PRIMA_CONFIG = r"C:\Users\jenia\Documents\models\models\primaLayout\config.yaml"
#PRIMA_MODEL = r'V:\FHSS-JoePriceResearch\RA_work_folders\Ethan_Simmons\layoutParser\primaLayout\model_final.pth'
PRIMA_MODEL = r"C:\Users\jenia\Documents\models\models\primaLayout\model_final.pth"
...
LSR_CONFIG = r"C:\Users\jenia\Downloads\models\models\customPubLayout\config.yaml"
#LSR_MODEL = r'V:\FHSS-JoePriceResearch\RA_work_folders\Ethan_Simmons\layoutParser\customPubLayout\model_final.pth'
LSR_MODEL = r"C:\Users\jenia\Downloads\models\models\customPubLayout\model_final.pth"
...
# tesseract path
#Change back when you give back the code
...
#Copied from LP-OCR-rotating.py, might be good to edit
...
#Also copied from LP-OCR-rotating.py
...
def isFlagSet(flag):
    """Checks whether a specific argument appears within sys.argv and removes it"""
    index = 0
    try:
...
def getTextRegions(layouts):
    ...
    regionPadding = 100  # some arbitrary value that seems to work pretty well (was 70 before)
    ...
    #NOTE: Eventually this will need to be taken into the rotating.
...
def addPageLabels(text, batchNames):
    ...
    for page in range(len(text)):
        for section in range(len(text[page])):
            text[page][section] += '\n-section end-\n'
        #TO-DO, re-add this line somehow
...
def getSettings(quiet = False):
    """Set all the appropriate global variables needed for later functions. See the global line or comment in main to see everything it modifies"""
...
def main():
    ...
    for i in range(4):
        ...
        print('\n\n#Finding Layouts, rotation' + str(i) + '#', flush = True)
        print('(expect some random output here; this is all from detectron2)\n')
        ...
        #Rotate images for next time around
        for item_num in range(len(text)):
            if isGibberish(text[item_num]):
                images[item_num] = rotateImage(images[item_num])
...
def obitMain():
    ...
    dirList = [x[0] for x in os.walk(workingDir)]  # finds all sub-folders within workingDir
